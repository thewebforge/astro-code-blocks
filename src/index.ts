...
export const codeBlocksAutoImport:
  | string
  | Record<string, (string | [string, string])[]> = {
  // "./src/components/Block.astro": [["default", CodeSnippetTagname]],
  "@thewebforge/astro-code-blocks/components": ["CodeBlock"],
};
...
export function remarkCodeSnippets(): Plugin<[], Root> {
  // @ts-potential-error - External lib
...
export default function CodeBlocksIntegration(
  opts: CodeBlocksUserConfig
): AstroIntegration[] {
    ...
    hooks: {
      "astro:config:setup": ({ config, updateConfig }) => {
        const newConfig: AstroUserConfig = {
          vite: {
            plugins: [vitePluginCodeBlocksUserConfig(userConfig, config)],
          },
          markdown: {
...
/** Expose the CodeBlocks user config object via a virtual module. */
function vitePluginCodeBlocksUserConfig(
  opts: CodeBlocksConfig,
  { root }: AstroConfig
): NonNullable<ViteUserConfig["plugins"]>[number] {
  const modules = {
    "virtual:codeblocks/user-config": `export default ${JSON.stringify(opts)}`,
    "virtual:codeblocks/project-context": `export default ${JSON.stringify({
      root,
    })}`,
  };
  ...
  return {
    name: "vite-plugin-codeblocks-user-config",
